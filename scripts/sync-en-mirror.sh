...
find en/mirror -name '*.md' -print0 | xargs -0 perl -0pi -e '
  ...
  s/Jira: Sprint Board/Jira: Sprint Board/g;
  s/Jira: Backlog/Jira: Backlog/g;
  s/Jira: Historie/Jira: History/g;
  ...
  s/Öffentliche Historie der Jira-Fläche \(ohne Jira-Cloud-Links\)\./Public history of the Jira area \(without Jira Cloud links\)\./g;
  ...
  s/Diese Seite bleibt nur als Kompatibilitätspfad erhalten\./This page only remains as a compatibility path\./g;
  s/Der aktuelle Zielort für erledigte Vorgänge und Sprint-Historie ist die\ngemeinsame Historie\./The current target for completed issues and sprint history is the shared history view\./g;
  s/## Weiter zur Historie/## Continue to History/g;
  s/## Sprint-Historie/## Sprint History/g;
  s/## Erledigte Vorgänge/## Completed Issues/g;
